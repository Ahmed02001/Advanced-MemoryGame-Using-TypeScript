...
function flip(card, index) {
    ...
    // if (card) {
    //   document.getElementById(`card-flip-${index}`).classList.toggle("flipped");
    // }
    if (card) {
...
function checkFinish() {
    if (P.initPrepare.cards.filter((card) => !card.clickable).length === P.NoOfCards) {
...
let cardContent = "";
P.initPrepare.cards.forEach((card, index) => {
    cardContent += `
<span class="col-sm-3 col-lg-2">
  <div id="toggle-card" class="card-flip">
    <div id="card-flip-${index}">
      ...
      <div class="back">
        <div class="card">
          <img src="../assets/images/${card.index}.jpg" alt="image...." data-holder-rendered="true" 
...
//     parentElement.classList.add("col-sm-3", "col-lg-2");
// }
const cardsContainer = document.getElementById("cards");
if (cardsContainer) {
